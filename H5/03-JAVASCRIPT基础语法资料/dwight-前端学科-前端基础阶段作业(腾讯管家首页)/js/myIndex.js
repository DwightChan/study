window.addEventListener('load', function() {
    ...
    */
    ...
    publishCopy.children[bannerIndex].style = 'dispay:block;z-index:1';
    ...
    // 上一张
    publishCopy.previous = function(ishide) {
        if (ishide) {
    ...
    publishCopy.next = function() {
        ...
        if (bannerLisCount < bannerLisCount) {
            bannerIndex = bannerIndex + bannerLisCount;
        }
    ...
    console.log(publishCopy.children.length / 3 * 2);
    console.log(bannerLisCount);


    for (var index = bannerLisCount; index < publishCopy.children.length / 3 * 2; index++) {
        let circleIndex = index % bannerLisCount;
        circlesBack.children[circleIndex].addEventListener('click', function() {
            // 先停止定时器
            clearTimeout(publishCopy.bannerTimer);
            publishCopy.previous(true);
            bannerIndex = circleIndex;;
            publishCopy.previous(false);
            // 开启定时器
            begineTimer();
